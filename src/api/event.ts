import {
  ...
  mapResponseXmlToJson,
  generateRequestElements,
  mapResponseObsToProperty
} from "../soap/request";
...
export class ONVIFEvent {
  ...
  static CreatePullPointSubscription(Filter?: any, InitialTerminationTime?: AbsoluteOrRelativeTimeType, SubscriptionPolicy?: any) {
    return createStandardRequestBodyFromString(generateRequestElements('tev:CreatePullPointSubscription')({
      tev_Filter: Filter,
      tev_InitialTerminationTime: InitialTerminationTime,
      tev_SubscriptionPolicy: SubscriptionPolicy
    }))
  ...
  CreatePullPointSubscription(Filter?: any, InitialTerminationTime?: AbsoluteOrRelativeTimeType, SubscriptionPolicy?: any) {
    return ONVIFEvent.CreatePullPointSubscription(Filter, InitialTerminationTime, SubscriptionPolicy).run(this.config).pipe(map((res) => {
      if (res.isOk()) {
        return new ONVIFPullPoint({...this.config, deviceUrl: res.unwrap().json.SubscriptionReference.Address})
      }
      throw res.unwrapFail();
    }));
  }
...
export class ONVIFPullPoint {
  ...
  static PullMessages(Timeout: string, MessageLimit: number) {
    return createStandardRequestBodyFromString(generateRequestElements('tev:PullMessages')({
      tev_Timeout: Timeout,
      tev_MessageLimit: MessageLimit
    }))
  ...
  static Seek(UtcTime: string, Reverse?: boolean) {
    return createStandardRequestBodyFromString(generateRequestElements('tev:Seek')({
      tev_UtcTime: UtcTime,
      tev_Reverse: Reverse
    }))
  ...
  PullMessages(Timeout: string, MessageLimit: number) {
    return ONVIFPullPoint.PullMessages(Timeout, MessageLimit).run(this.config)
  }
  ...
  Seek(UtcTime: string, Reverse?: boolean) {
    return ONVIFPullPoint.Seek(UtcTime, Reverse).run(this.config)
  }
  ...
  SetSynchronizationPoint() {
    return ONVIFPullPoint.SetSynchronizationPoint().run(this.config)
  }
   ...
   */
  Unsubscribe() {
    return ONVIFPullPoint.Unsubscribe().run(this.config)
  }
}
